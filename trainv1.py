...
def train_cifar():
    ...
    pg = ProcessGroup()
    chunk_size = ChunkManager.search_chunk_size(model, 8 * 1024**2, 512, filter_exlarge_params=True)
    chunk_manager = ChunkManager(chunk_size, pg, enable_distributed_storage=True, init_device=GeminiManager.get_default_device('cuda'))
